...
parser.add_argument('--experiment_name', type=str, default='model_2x2-7x7(HCI)',
                    help='name for the experiment to run')
parser.add_argument('--resume_epoch', type=int, default='38625', help='epoch to resume')
...
parser.add_argument('--disp_range', type=int, default=4, help='depth range for psv')
parser.add_argument('--num_planes', type=int, default=50, help='step number for psv')
...
def test(test_loader, model):
    ...
    with torch.no_grad():
        ...
        for k, batch in enumerate(test_loader):
            ...
            flops, params = profile(model, inputs=(idx_src, input, idx_target, opt,))
            ...
            if not opt.crop:
                ...
                print('time {: .5f}'.format(end_time - start_time))
        ...
        print('Running time {: .5f}'.format(cost_time / (len(test_loader) - 1)))
...
if __name__ == '__main__':
    ...
    if opt.train_dataset == 'SIG':
        ...
        test_datasets = ['HCI_old', 'HCI', 'Inria_DLFD']
